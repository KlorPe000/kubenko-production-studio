...
export default function Home() {
  ...
  // Smart portfolio item opener
  const handlePortfolioItemClick = (item: PortfolioItem) => {
    const hasPhotos = item.photos && item.photos.length > 0;
    const hasVideo = Boolean(item.videoUrl);
    
    if (hasPhotos && hasVideo) {
      // Both photo and video exist - show modal for choice
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            ...
            .map((item, index) => (
              <Card 
                key={item.id} 
                className={`bg-gradient-to-br from-green-50 to-white rounded-lg md:rounded-2xl overflow-hidden shadow-md md:shadow-xl hover:shadow-lg md:hover:shadow-2xl transition-all duration-500 transform hover:scale-[1.02] slide-in-bottom delay-${(index + 1) * 100} cursor-pointer`}
                onClick={() => handlePortfolioItemClick(item)}
              >
                <div className="relative group overflow-hidden">
                  {(item.categoryPreview || item.photoThumbnail || item.videoThumbnail || (item.videoUrl && getYouTubeThumbnail(item.videoUrl))) ? (
                    <>
                      <div className="w-full">
                        <img
                          src={item.categoryPreview || item.photoThumbnail || item.videoThumbnail || (item.videoUrl ? getYouTubeThumbnail(item.videoUrl) : '') || ''}
                ...
                <CardContent className="p-3 md:p-6">
                  <h3 className="font-display text-sm md:text-lg font-semibold text-wedding-charcoal mb-2 md:mb-3">{item.categoryName}</h3>
